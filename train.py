...
if __name__ == '__main__':
    ...
    for e in range(args.e):
        ...
        loss.backward()
        optimizer.step()
        train_acc = calc_f1(pred[train_nodes].detach().cpu().numpy(), label[train_nodes].detach().cpu().numpy())
        model.eval()
        pred = model(features)
        val_acc = calc_f1(pred[val_nodes].detach().cpu().numpy(), label[val_nodes].detach().cpu().numpy())
        test_acc = calc_f1(pred[role['te']].detach().cpu().numpy(), label[role['te']].detach().cpu().numpy())
        if val_acc > best_val:
            best_e = e
            best_val = val_acc
            best_test = test_acc
    ...
    accs, f1s = calc_class_level_acc(pred[role['te']].detach().cpu().numpy(), best_pred, torch.max(label).item() + 1)
